...
deg= readRDS("deg.rds")
markers= readRDS("all_markers_cell_assigned.rds")
...
output$sc1c2oup.png <- downloadHandler( 
  ...
  output$deg_table = DT::renderDataTable(server=FALSE,{
    ...
    dt = dt[dt$cell_type==input$celltype,]
    
    dt = dt[dt$avg_logFC>=input$log2fc|dt$avg_logFC<=-input$log2fc ,]
    
    dt = datatable(dt, rownames = FALSE, extensions = "Buttons",
  ...
  output$markers_table = DT::renderDataTable(server=FALSE,{
    ...
    dt_marker = dt_marker[dt_marker$cluster==input$celltype_markers,]
    
    
    dt_marker = dt_marker[dt_marker$avg_log2FC>=input$log2fc_markers ,]
    # dt_marker= dt_marker %>% dplyr::select(gene, avg_log2FC, p_val, p_val_adj, dplyr::everything())
    dt_marker = datatable(dt_marker, rownames = FALSE, extensions = "Buttons",
                     filter = list(position = 'top', clear = TRUE),
                     options = list(pageLength = 10, dom = "Bfrtip",
                                    buttons = c("copy", "csv", "excel", "pdf"),
                                    scrollX = TRUE))
